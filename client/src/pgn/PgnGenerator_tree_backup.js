...
class PgnGenerator {
   ...
   */
    async generateTreePGN(lines, chapterName, engineClient = null) {
        ...
        // Generate tree PGN with variations (handle inline annotations if configured)
        let treePgn;
        if (this.config.annotationStyle === 'inline') {
            treePgn = await this.generateTreeMoveSequenceWithInlineStats(variationTree, engineClient);
        } else {
            treePgn = await this.generateTreeMoveSequence(variationTree, engineClient);
        }
   ...
   */
    ...
    async generateTreeMoveSequence(variationTree, engineClient = null) {
        ...
        for (let i = 0; i < mainLine.moves.length; i++) {
            ...
            if (variationTree.variations && variationTree.variations.length > 0) {
                // Find variations that diverge at this move position
                const variationsAtThisPosition = variationTree.variations.filter(variation => {
                    return variation.moves && variation.moves.length > i &&
                           variation.moves[i].san !== mainLine.moves[i].san;
                });

                for (const variation of variationsAtThisPosition) {
                    // Only show the divergent part of the variation
                    const divergentMoves = variation.moves.slice(i);
                    if (divergentMoves.length > 0) {
                        let variationPgn = '';

                        // Format variation moves with proper numbering
                        for (let j = 0; j < divergentMoves.length; j++) {
                            const varMove = divergentMoves[j];
                            const varMoveNumber = Math.floor((i + j) / 2) + 1;
                            const isVarWhiteMove = (i + j) % 2 === 0;

                            if (isVarWhiteMove) {
                                variationPgn += `${varMoveNumber}. ${varMove.san}`;
                            } else {
                                variationPgn += `${varMoveNumber}...${varMove.san}`;
                            }

                            if (j < divergentMoves.length - 1) {
                                variationPgn += ' ';
                            }
                        }
    ...
    /**
     * Generate tree-structured move sequence with inline statistics
     * @param {Object} variationTree - Tree structure from buildVariationTree
     * @param {Object} engineClient - Optional engine for line completion
     * @returns {string} PGN formatted tree with variations and inline stats
     */
    async generateTreeMoveSequenceWithInlineStats(variationTree, engineClient = null) {
        if (!variationTree || !variationTree.mainLine) {
            return '';
        }
        ...
        // Create a map of move san to playrate from mainLine's likelihoodPath
        const playrateMap = new Map();
        if (mainLine.likelihoodPath) {
            for (const pathMove of mainLine.likelihoodPath) {
                if (pathMove.san && pathMove.playrate !== undefined) {
                    playrateMap.set(pathMove.san, pathMove.playrate);
                }
            }
        }

        // Generate main line moves with inline stats
        for (let i = 0; i < mainLine.moves.length; i++) {
            const move = mainLine.moves[i];
            const moveNumber = Math.floor(i / 2) + 1;
            const isWhiteMove = i % 2 === 0;
            ...
            const playrate = playrateMap.get(move.san);
            if (playrate !== undefined) {
                const playrateFormatted = this.formatPercentage(playrate);
                pgn += `${move.san}{${playrateFormatted}}`;
            } else {
                pgn += move.san;
            }
            ...
            if (variationTree.variations && variationTree.variations.length > 0) {
                ...
                for (const variation of variationsAtThisPosition) {
                    ...
                    if (divergentMoves.length > 0) {
                        ...
                        // Create playrate map for this variation
                        const varPlayrateMap = new Map();
                        if (variation.likelihoodPath) {
                            for (const pathMove of variation.likelihoodPath) {
                                if (pathMove.san && pathMove.playrate !== undefined) {
                                    varPlayrateMap.set(pathMove.san, pathMove.playrate);
                                }
                            }
                        }

                        // Format variation moves with proper numbering and inline stats
                        for (let j = 0; j < divergentMoves.length; j++) {
                            const varMove = divergentMoves[j];
                            const varMoveNumber = Math.floor((i + j) / 2) + 1;
                            const isVarWhiteMove = (i + j) % 2 === 0;
                            ...
                            if (isVarWhiteMove) {
                                variationPgn += `${varMoveNumber}. `;
                            } else {
                                variationPgn += `${varMoveNumber}...`;
                            }

                            // Add move with inline playrate if available
                            const varPlayrate = varPlayrateMap.get(varMove.san);
                            if (varPlayrate !== undefined) {
                                const varPlayrateFormatted = this.formatPercentage(varPlayrate);
                                variationPgn += `${varMove.san}{${varPlayrateFormatted}}`;
                            } else {
                                variationPgn += varMove.san;
                            }
        ...
        if (engineClient && this.config.ENGINEFINISH) {
            try {
                const lastPosition = mainLine.fen || 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1';
                const engineMove = await engineClient.getBestMove(lastPosition, this.config.ENGINEDEPTH);
                if (engineMove) {
                    const nextMoveNumber = Math.floor(mainLine.moves.length / 2) + 1;
                    const isWhite = mainLine.moves.length % 2 === 0;
                    if (isWhite) {
                        pgn += `${nextMoveNumber}. ${engineMove}`;
                    } else {
                        pgn += engineMove;
                    }
                }
    ...
    formatCombinedAnnotations(lines) {
        ...
        if (totalGames > 0) {
            ...
            annotations += `\n${winrateDescription}: +${winratePercent}% over ${gamesFormatted} games`;
        }
    ...
    async generateMoveSequenceWithInlineStats(line, engineClient = null) {
        ...
        let moveSequence = '';
        // Create a map of move san to playrate from likelihoodPath
        const playrateMap = new Map();
        if (line.likelihoodPath) {
            for (const pathMove of line.likelihoodPath) {
                if (pathMove.san && pathMove.playrate !== undefined) {
        ...
        for (let i = 0; i < line.moves.length; i++) {
            ...
            // Add move with inline playrate if available in likelihoodPath
            const playrate = playrateMap.get(move.san);
            if (playrate !== undefined) {
                const playrateFormatted = this.formatPercentage(playrate);
