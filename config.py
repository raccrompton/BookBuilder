...
MOVES = 5
#The number of most played moves to search over for the best move, and the number of opponent moves to consider as continuations (minimum 5, maximum 12)

#MOVE SELECTION SETTINGS
DEPTHLIKELIHOOD = 0.05
#once cumulative line likelihood reaches this threshold, no futher continuation moves will be added (in percentage so 0.0025 = 0.25% of occuring in your games)
ALPHA = 0.001
#The confidence interval alpha (EG 0.05 = 95% CI), for deciding the lower bounds of how good a move's winrate is. If this is high, moves with less data will be picked more often
MINPLAYRATE = 0.001
...
#the filepath where the engine is stored on your computer, so it can be accessed. Keep the 'r' character and the quotes around the file path
CAREABOUTENGINE = 1
#care about engine eval of position or engine finishing = 1, dont care = 0
ENGINEDEPTH = 10
#how deep engine should evaluate best moves. For many openings a high depth is recommended, but takes longer.
ENGINEFINISH = 1
#if we want the engine to complete lines to cumulative likelihood where data is insufficient, 1. Otherwise 0, and lines will end where there's no good human data to pick from
SOUNDNESSLIMIT = -99
